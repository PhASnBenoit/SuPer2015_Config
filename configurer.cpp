...
bool Configurer::actuAffichage(){
    ...
    //affecter des texte par défaut pour les labels de modif de mdp
    ...
    if(!listePersonne.empty()){
        for(int i = 0; i < listePersonne.count(); i++){
            ...
            ui->cBoxPersModif->setEnabled(true);
            ...
            ui->cBoxPersSupp->setEnabled(true);
        ...
        ui->cBoxPersModif->setEnabled(false);
        ui->cBoxPersSupp->addItem("Aucune Personne");
        ui->cBoxPersSupp->setEnabled(false);
    ...
    if(!listeLecteur.empty()){
        for(int i = 0; i < listeLecteur.count(); i++){
            ...
            //charger la liste du lieu correspondant au lecteur
    ...
    if(!listeLieu.empty()){
       ui->cBoxLieuxForLecteur_Modif->setEnabled(true);
       ...
       //afficher lieux non découvert
       ui->cBoxLieuxForLecteur_Modif->addItem("Aucun lieu découvert");
       ui->cBoxLieuxForLecteur_Modif->setEnabled(false);
    }

    //simuler selection lecteurSuppr pour affichage auto
    on_cBoxLectSupp_activated(0);

    //simuler selection lecteurModif pour affichage auto
...
{
    //effectue la suite si il y a des personnes dans la combobox
    if(ui->cBoxPersonneLier->currentText() != "Aucune personne à lier"){
        index=0;
        index++;
...
{
    //test si les deux combobox contiennent des données
    if(ui->cBoxAffecterBadge->currentText() != "Aucun badge" && ui->cBoxPersonneLier->currentText() != "Aucune personne à lier"){
    ...
    if(!lier){
        ...
        QMessageBox::warning(0, tr("Attention : requete impossible"),
                     tr("Aucun personne ou badge présent.\nVerifier les champs.\nErreur 002."),
                              QMessageBox::Ok);
...
{
    /*     modif le 03-02-15     */
    if(ui->cBoxBadgeSupp->currentText() != "Aucun badge"){
...
void Configurer::on_btOkDelier_clicked()
{
    int num_badge;
    bool delier =false ;
    //test si le champ contient des badges
    if(ui->cBoxBadgeSupp->currentText()!="Aucun badge"){

    //récupération des informations dans les champs
    num_badge = ui->cBoxBadgeSupp->currentText().toInt();
    }
    else{
    num_badge = -1;
    ...
    if(num_badge == -1){  //Problème
        QMessageBox::warning(0, tr("Attention : erreur"),
                     tr("Impossible de trouver la personne lié à ce badge.\nVerifier les champs.\nErreur 001c."),
                              QMessageBox::Ok);
    }else{

    //requête
     delier = pBdd->setDelier(num_badge);
    }
    if(!delier){
        //erreur
        QMessageBox::warning(0, tr("Attention : requete impossible"),
...
{
    if(ui->cBoxSupp->currentText() != "Aucun badge"){
        ...
        ui->txtInfoBadgeSupp->clear();
        ui->txtInfoBadgeSupp->textCursor().insertText("Numéro de badge "+num_badge+" avec l'identifiant : "+id_badge+".\nDate de mise en service : "+dateMiseEnService+". Date de changement Pile : "+dateChangePile);
    }//if
}
...
void Configurer::on_cBoxModif_activated(int index)
{
    ...
    if(ui->cBoxModif->currentText() != "Aucun badge"){
...
{
    //test la présence de badge à modif
    if(ui->cBoxModif->currentText() != "Aucun badge"){
...
{
    if(ui->cBoxAffecterBadge->currentText() != "Aucun badge"){
        ...
        QString dateChangePile = listeBadge.at(index).dateChangePile;
        ui->txtInfoBadge->clear();
        ui->txtInfoBadge->textCursor().insertText("Numéro de Badge "+num_badge+" avec l'identifiant : "+id_badge+".\nDate de mise en service : "+dateMiseEnService+". Date de changement Pile : "+dateChangePile);
...
void Configurer::on_cBoxLectSupp_activated(int index)
{
    ...
    //ajout dans la box d'info du badge
    ui->txtInfoLectDel->clear();
    ui->txtInfoLectDel->textCursor().insertText("Lecteur n° : "+numLecteur+"\nAdresse IP : " + ipLecteur + "\nNuméro du lieu associé : "+ lieuLecteur +"\n\n");
}
